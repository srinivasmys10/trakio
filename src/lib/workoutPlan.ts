...
import type { WorkoutSlot, NewWorkoutSlot, ExerciseLibraryItem, ExerciseType } from '../types'
...
function rowToSlot(row: WorkoutRow): WorkoutSlot {
  ...
  return {
    ...
    weekday:      row.weekday    as WorkoutSlot['weekday'],
    exercise_id:  row.exercise_id,
    custom_sets:  row.custom_sets  ?? null,
    custom_reps:  row.custom_reps  ?? null,
    notes:        row.notes,
...
export async function addWorkoutSlot(slot: Omit<NewWorkoutSlot, 'sort_order'>): Promise<WorkoutSlot> {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      custom_sets: slot.custom_sets ?? null,
      custom_reps: slot.custom_reps ?? null,
      notes:       slot.notes,
